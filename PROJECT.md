...
└── src/                            （23 个文件）
    ...
    │   ├── resume/page.tsx        ← ★ 简历终端（macOS 终端窗 + 打字会话，点终端卡进来，见 4.7.11）
...
##### ★ `/resume` —— 简历终端（2026-09-24 深夜 v2：完全复刻 macOS 终端）

页面壳 `src/app/resume/page.tsx`（服务端组件，挂 metadata）；终端本体
`src/components/resume/terminal-session.tsx`（**客户端组件**）。
小潘的要求：「完全完全复刻 zhangyu.dev/resume 那个苹果系统的命令行，
哪怕左上角叉/缩小那些按钮，动效一定要很好」—— 把参考站的 page chunk
反编译扒出了它的三积木机制，照此 1:1 复刻：
...
- **打字动效 = 三块积木**（照参考站 Typed / TypedText / TypedContent 复刻）：
  ...
  **逐字打出**（110~190ms/字随机抖动，打字中显示 `~` + 闪烁 `█`，打完变粗斜体）；
  `TypedContent` 立即显示、停 1.4s 放行下一个；每个新元素 `scrollIntoView(smooth)`。
- **SSR / reduce 兜底**：`useSyncExternalStore` server snapshot 返回 true →
  服务端 / 无 JS 整页静态直出；reduced-motion 下 `Typed` 挂载即放行全部 ——
  **内容不依赖动画才可见**（§6.4 老规矩）。
...
> 🔴 `Math.random()` 只出现在打字节奏的 setTimeout 回调里，绝不参与渲染。
...
> 📌 **2026-09-24 深夜第五版（姿势）**：小潘点名「猫咪的形象还是没改成侧躺啊……
> 一定要侧躺，登录页就不用改了」—— 第 2 格整只翻转成**侧躺**（身体横面包 +
> 头在右端支起 + 前爪伸向右前方 + 尾巴左后卷勾），登录页那只**原样没动**。

> 📌 **2026-09-24 照小潘给的参考图（写实风黑猫插画）整体重做了一次**：
> 姿势从「两只都趴着」改成「第 2 格侧躺（原趴卧）/ 登录页坐着」，
...
- **侧躺的身体是横面包，头要「支起来」** —— 头底和身体上缘有一段重叠（头画在
  身体后面那层之前），下巴微抬才有「枕着手臂看你」的懒洋洋读法；
  前爪一前一后**从胸口底下伸向右前方**，趾缝深色短线点出来。
- **尾巴别画成自交的圈** —— 中间留个洞，看着像杯子的把手。
  第 2 格那条从左后方绕出来向上卷个小勾；登录页那条**搭在框沿上、尾巴尖翘起来**，
  这条一改「坐姿」的读法立刻就出来了。
